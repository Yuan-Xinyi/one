...
The distilled policy replaces the PPO policy as the interior controller of the hybrid architecture. The switch is largely internalized---the fraction of tasks on which the runtime switch still improves over $\pi_{\mathrm{D}}$ alone drops from $22.8\%$ to $6\%$---yet a residual benefit remains, so the deployed configuration keeps the hybrid form with retuned thresholds (Section~\ref{sec:coordination}). The loop converges after roughly a dozen rounds: the standalone student improves monotonically while the switch-benefit fraction falls, and the last two rounds change the result by less than $0.1$\,pp, at which point the stopping criterion fires. By construction the student converges toward its teacher, so its ceiling is the hybrid controller itself; this is acceptable because internalization, not surpassing the teacher, is the goal here, and because the remaining losses of the deployed system lie elsewhere (Section~\ref{sec:attribution}). In exploratory experiments we also upgraded the expert's shell branch to an exact lookahead search over candidate actions in the simulator; at convergence this variant leads the search-free student by only $0.3$\,pp standalone and $0.4$\,pp as the deployed system---most of what the search teacher provides is recovered by simply iterating the search-free loop longer---so we exclude it from the final method, as the added machinery and hyperparameters are disproportionate to the gain, and the dominant improvement comes from initialization (Section~\ref{sec:rankedresults}). Algorithms~\ref{alg:exit} and~\ref{alg:expert} summarize the two levels separately, in the notation of expert iteration \cite{anthony2017thinking}: Algorithm~\ref{alg:exit} is the outer improvement loop, and Algorithm~\ref{alg:expert} is the expert it repeatedly rebuilds---the hybrid behavior map around the current apprentice. The expert's interior branch \emph{is} the apprentice, so improving the apprentice improves the expert; the loop terminates when the runtime switch no longer improves on the apprentice, i.e., the improvement operator is exhausted.}
...
Before extending the initialization, we ask where the system still loses. All attribution numbers below are measured with the deployed controller of Section~\ref{sec:distillresults}. We define the catastrophic tail as the Easy-bucket tasks recovering less than $50\%$ of $\ell^{\mathrm{ref}}$ ($n{=}428$); although small in number, these tasks carry $41\%$ of the total shortfall of the deployed system. Three findings characterize this tail. First, the deaths are dominated by the tool-orientation cone rather than the joint limits: $68.2\%$ of the tail terminates by cone violation, $19.9\%$ by joint limits, and $11.2\%$ by collision, with $93\%$ terminating within the first $50$ steps (median $33$)---the near-boundary rescue skill that the hybrid controller provides is no longer the binding constraint. Second, the failures are attributable to the initial branch, not to control: re-running the \emph{same} controller from the classical-label seed recovers $42.8\%$ of the tail to at least $90\%$ of the reference, and starting the remaining stuck tasks from the per-task best stored candidate recovers a further $66.3\%$ of them; in total, over $70\%$ of the catastrophic tail recovers to at least $90\%$ from a better start, and another $16\%$ recovers partially. Third, the diffusion samples themselves contain the better starts: for the same task, the first IK-valid sample performs no better than the per-task average over eight samples ($90.9\%$ vs.\ $90.9\%$ of the reference on the full set), i.e., the retained sample is effectively a random draw, while an oracle choosing the best of the eight would recover $100.3\%$. Initialization quality is a geometric property of the candidate, fully observable before the motion starts; the selection headroom is real and, as the next sections show, largely realizable.}

\rev{\subsubsection{Internalizing the Switch}\label{sec:distillresults}
Table~\ref{tab:distill} evaluates the distilled policy of Section~\ref{sec:distill} under the conference initialization (first IK-valid DP seed). Two observations stand out. First, the standalone distilled policy $\pi_{\mathrm{D}}$ recovers $89.1\%$ of the reference length, surpassing the conference PPO policy ($85.0\%$) by $4$\,pp and approaching the conference hybrid \emph{system} ($90.6\%$) as a single network. Second, the difficulty gradient reported in the conference version---the learned policy retreating to classical parity on Difficult tasks---disappears: $\pi_{\mathrm{D}}$ is strongest exactly on the Difficult bucket ($94.8\%$), because the expert supplies, by imitation, precisely the near-boundary competence that PPO could not discover through exploration. Re-hosting $\pi_{\mathrm{D}}$ as the interior controller of the hybrid architecture retains a residual benefit (the runtime switch still helps on $8.5\%$ of the tasks) and lifts the deployed system to $91.0\%$. The hybrid architecture is thus best understood as a policy-improvement operator: most of its value can be distilled into a single network, while the runtime switch contributes a diminishing remainder.}
...
& $\pi_{\mathrm{D}}$ (distilled)      & 0.56 / 0.25 / 0.01 / 1.51 & 89.1 / 26.2 / 0.8 / \phantom{0}985.9 \\
& Hybrid (conf.)            & 0.57 / 0.25 / 0.01 / 1.45 & 90.6 / 23.1 / 0.8 / 1000.8 \\
& Hybrid($\pi_{\mathrm{D}}$)          & \limebox{0.57} / 0.25 / 0.01 / 1.51 & \limebox{91.0} / 23.6 / 0.8 / \phantom{0}985.6 \\
\midrule
\multirow{4}{*}{\shortstack{Easy\\{\scriptsize ($n{=}2642$)}}}
& RL (conf.)                & 0.74 / 0.32 / 0.01 / 1.45 & 75.7 / 31.4 / 0.8 / \phantom{0}143.5 \\
& $\pi_{\mathrm{D}}$ (distilled)      & 0.79 / 0.30 / 0.01 / 1.45 & 80.1 / 28.8 / 0.8 / \phantom{0}143.7 \\
& Hybrid (conf.)            & 0.81 / 0.28 / 0.01 / 1.45 & 82.4 / 26.2 / 0.8 / \phantom{0}143.5 \\
& Hybrid($\pi_{\mathrm{D}}$)          & \limebox{0.81} / 0.28 / 0.01 / 1.45 & \limebox{82.7} / 26.3 / 0.8 / \phantom{0}144.8 \\
\midrule
\multirow{4}{*}{\shortstack{Medium\\{\scriptsize ($n{=}4795$)}}}
& RL (conf.)                & 0.53 / 0.17 / 0.01 / 1.26 & 87.2 / 24.9 / 1.2 / \phantom{0}243.2 \\
& $\pi_{\mathrm{D}}$ (distilled)      & 0.56 / 0.15 / 0.01 / 1.12 & 91.1 / 19.9 / 1.2 / \phantom{0}233.3 \\
& Hybrid (conf.)            & 0.57 / 0.14 / 0.01 / 1.26 & 92.3 / 16.9 / 1.2 / \phantom{0}235.3 \\
& Hybrid($\pi_{\mathrm{D}}$)          & \limebox{0.57} / 0.13 / 0.01 / 1.23 & \limebox{92.7} / 16.8 / 1.2 / \phantom{0}233.3 \\
\midrule
\multirow{4}{*}{\shortstack{Difficult\\{\scriptsize ($n{=}2563$)}}}
& RL (conf.)                & 0.32 / 0.11 / 0.01 / 1.28 & 90.5 / 30.5 / 2.2 / 1017.6 \\
& $\pi_{\mathrm{D}}$ (distilled)      & 0.33 / 0.10 / 0.01 / 1.51 & 94.8 / 31.1 / 2.4 / \phantom{0}985.9 \\
& Hybrid (conf.)            & 0.33 / 0.09 / 0.01 / 1.28 & 95.7 / 27.2 / 2.4 / 1000.8 \\
& Hybrid($\pi_{\mathrm{D}}$)          & \limebox{0.33} / 0.10 / 0.01 / 1.51 & \limebox{96.3} / 28.7 / 2.4 / \phantom{0}985.6 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
...
Table~\ref{tab:ranked} reports the full method: the hybrid controller with $\pi_{\mathrm{D}}$, initialized by the ranked selection of Section~\ref{sec:ranking}. Ranked selection lifts the deployed system from $90.9\%$ to $\mathbf{98.2\%}$ of the reference length (paired difference $+7.30\pm0.23$\,pp), corresponding to a mean path length of $0.63$\,m---$39\%$ longer than the classical baseline and $10\%$ longer than the conference hybrid. The gains concentrate where the attribution predicted: the Easy bucket rises from $83.0\%$ to $93.9\%$, and the mean recovery of the catastrophic tail rises from $30\%$ to $83.7\%$, with $80\%$ of those tasks leaving the sub-$50\%$ regime. On Difficult tasks the full method exceeds the reference pool ($101.6\%$), confirming that the frozen $\ell^{\mathrm{ref}}$ is a reference rather than a ceiling. The oracle best-of-batch bound is $102.9\%$; the ranker realizes $61\%$ of the selection headroom without rolling out a single candidate.}
...
\midrule
\multirow{3}{*}{\shortstack{All\\{\scriptsize ($n{=}10000$)}}}
& First IK-valid (conf.)  & 0.57 / 0.25 / 0.01 / 1.47 & 90.9 / 22.1 / 0.9 / 605.5 \\
& Ranked ($24{+}1$ candidates) & \limebox{0.63} / 0.26 / 0.01 / 1.58 & \limebox{98.2} / 19.8 / 0.9 / 683.8 \\
& \textit{Best-of-batch (oracle)} & \textit{0.66} & \textit{102.9} \\
\midrule
\multirow{2}{*}{\shortstack{Easy\\{\scriptsize ($n{=}2642$)}}}
& First IK-valid (conf.)  & 0.82 / 0.27 / 0.01 / 1.47 & 83.0 / 25.9 / 0.9 / 171.5 \\
& Ranked ($24{+}1$ candidates) & \limebox{0.93} / 0.21 / 0.01 / 1.58 & \limebox{93.9} / 18.2 / 0.9 / 171.3 \\
\midrule
\multirow{2}{*}{\shortstack{Medium\\{\scriptsize ($n{=}4795$)}}}
& First IK-valid (conf.)  & 0.57 / 0.14 / 0.01 / 1.23 & 92.5 / 17.0 / 1.3 / 222.4 \\
& Ranked ($24{+}1$ candidates) & \limebox{0.61} / 0.12 / 0.01 / 1.30 & \limebox{98.7} / 12.6 / 1.3 / 260.7 \\
\midrule
\multirow{2}{*}{\shortstack{Difficult\\{\scriptsize ($n{=}2563$)}}}
& First IK-valid (conf.)  & 0.33 / 0.09 / 0.01 / 1.32 & 96.0 / 24.1 / 2.3 / 605.5 \\
& Ranked ($24{+}1$ candidates) & \limebox{0.35} / 0.11 / 0.01 / 1.43 & \limebox{101.6} / 29.4 / 2.3 / 683.8 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
    \item[Note 1] Best-of-$K$ oracle prefixes over the $w{=}1.5$ candidates: $K{=}2$: $95.1\%$, $K{=}4$: $98.3\%$, $K{=}8$: $100.3\%$; mean over valid candidates: $90.9\%$ ($\approx$ first-valid, i.e., candidates are exchangeable).
...
\midrule
First IK-valid (no selection)                       & 90.9 \\
\midrule
...
\quad $16$ at $w{=}1.5$, $+$ fallback               & 97.9 \\
...
Hybrid($\pi_{\mathrm{D}}$) $+$ first IK-valid & 90.2 & 90.9 \\
Hybrid($\pi_{\mathrm{D}}$) $+$ ranked         & \limebox{98.3} & \limebox{98.2} \\
