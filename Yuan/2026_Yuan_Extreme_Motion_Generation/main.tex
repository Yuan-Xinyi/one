...
% (\rst) restructuring pass: additions in this color, removals kept via \sout.
\definecolor{rstcolor}{rgb}{0.0,0.50,0.55}
\newcommand{\rst}[1]{{\color{rstcolor}#1}}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
This work studies \textit{extreme motion generation}, which aims to maximize the \sout{Cartesian path length along a pre-defined trajectory within the manipulator's workspace} \rst{continuously executable Cartesian path length along a pre-defined trajectory, requiring every kinematic and task constraint to be satisfied throughout the entire execution rather than at isolated waypoints}. This objective is important in industry as path-following is fundamental to a large variety of tasks such as surface coating and welding. More critically, extreme motion enables a fixed-base manipulator to exploit the kinematic capability under limited reachability. However, such exploitation is challenging in practice, as the manipulator must actively avoid the safety boundary through execution, which is inherently a long-horizon problem. \xinyi{Accordingly, we claim that a learning-based policy capable of long-horizon decision-making should maximize exploitation within the interior safe region while carefully avoiding joint-limit violations near the boundary. In detail, we first construct an expert hybrid policy whose model-based controller takes over near the joint limits, where the learning-based policy fails under sparse data coverage. We then distill this expert into a single learning-based policy, \sout{which in turn constitutes a stronger expert in the next iteration,} \rst{iteratively correcting the policy against the fixed hybrid expert on the states it visits,} thereby closing the expert-iteration loop.} The initial joint configuration \xinyi{selection} is \xinyi{based on} the conditional diffusion-based sampling, which improves the achievable path length based on the learned motion prior. We evaluate the proposed framework on 5,000 held-out straight-line path-following tasks with a 7-DoF Franka FR3, \sout{\xinyi{recovering 91.2\% of the per-task reference length and extending the average path length by 39\% over the model-based baseline.}} \rst{With the initial configuration fixed, the distilled controller extends the average path length by $28\%$ over the classical null-space controller; ranked initialization then lifts the full system from $83.7\%$ to $91.2\%$ of the per-task reference length, an empirical enumeration-classical reference rather than a task upper bound.} The project website and related videos of this paper can be found at \url{https://yuan-xinyi.github.io/extreme-motion-generation/}.
...
\rst{In summary, this work makes three contributions.
\begin{itemize}
...
\item We construct a hybrid null-space expert that combines the long-horizon learned policy in the well-sampled interior with the reliable classical controller near the joint limits, and progressively internalize this expert into a single learning-based policy through expert iteration.
\item We introduce a diffusion prior with a learned scorer that selects a favorable self-motion-manifold branch as the initial configuration, and analyze the contribution of initialization, control, switching, and expert iteration on a large held-out task set.
\end{itemize}}
...
We formalize the above task as follows. Consider a manipulator with \sout{$n$} \rst{$N$} degrees of freedom, where \sout{$\mathbf{q} \in \mathbb{R}^{n}$} \rst{$\mathbf{q} \in \mathbb{R}^{N}$} denotes the joint configuration \rst{(we write $N$ for the number of joints so that $\mathbf{n}$ is reserved for the task-plane normal below)}. Let $\mathbf{p}(\mathbf{q}) \in \mathbb{R}^{3}$ denote the end-effector position and $\mathbf{z}(\mathbf{q}) \in \mathbb{R}^{3}$ denote the TCP local $z$-axis. A straight-line path-following task is specified by the condition vector:
\begin{equation}
\label{cond}
...
\rst{The pre-defined path is the ray $\mathbf{p}(s)=\mathbf{p}_{0}+s\,\mathbf{d}$ parametrized by the traveled arc length $s\ge 0$, and the reference position at step $t$ is $\mathbf{p}_{t}=\mathbf{p}(s_{t})$ with $s_{t}$ advancing along $\mathbf{d}$.} Starting from an initial configuration $\mathbf{q}_{0}^{*}$, the policy $\pi$ determines the joint motion at every control step to maximize the path-following length, producing a joint trajectory $\{\mathbf{q}_{t}^{*}\}_{t=0}^{T}$ initialized at $\mathbf{q}_{0}^{*}$, where $T$ is the termination step at which any safety constraint is first violated. \rst{Concretely, $T=\min\{t: \|\mathbf{p}(\mathbf{q}_{t}^{*})-\mathbf{p}_{t}\|>\epsilon_{\text{pos}},\ \text{or}\ \arccos(\mathbf{z}(\mathbf{q}_{t}^{*})^{\top}\mathbf{n})>\theta_{\max},\ \text{or}\ \mathbf{q}_{t}^{*}\notin[\mathbf{q}_{\min},\mathbf{q}_{\max}],\ \text{or}\ \mathbf{q}_{t}^{*}\ \text{is in self-collision}\}$, and the path-following length is the arc length reached at the last feasible waypoint, $\mathcal{L}=s_{T-1}$. In our implementation the position tolerance is enforced as a bound on the lateral deviation from the ray ($\epsilon_{\text{pos}}{=}0.02$\,m).} This task can be formalized as a constrained optimization problem as:
\begin{align}
...
\end{align}
where $\mathcal{L}$ denotes the path-following length, $\mathbf{q}_0^*$ and $\pi$ are the initial configuration and the control policy to be optimized, and $(\mathbf{p}_0, \mathbf{d}, \mathbf{n})$ specify the task conditions as above. The $\epsilon_{\text{pos}}$, $\theta_{\max}$, and $[\mathbf{q}_{\min}, \mathbf{q}_{\max}]$ are the position tolerance, orientation tolerance, and joint limits, respectively. \rst{The first two are task constraints that keep the end-effector on the line and the tool axis within the orientation cone, whereas the joint-limit constraint is the kinematic constraint that, in practice, is the one most often met first and thus terminates the motion. The commanded joint velocity is the sum of a task term and a null-space term: the null-space term is bounded by the scaling $a_{\max}$ introduced below, while the task term is regularized by the damped pseudo-inverse, which limits the joint velocity near singularities. We therefore do not list a separate velocity-limit constraint.}

During this process, since only the $3$-DoF end-effector position must be precisely tracked, the remaining freedom makes the manipulator kinematically redundant for \sout{$n>3$} \rst{$N>3$}. This redundancy is resolved in the null space of the position Jacobian, which decomposes the commanded joint velocity into a task term and a null-space term,
\begin{equation}
\label{eq:nullspace}
...
\end{equation}
where the secondary command $\boldsymbol{\xi}$ combines a directional-manipulability term $w_{\mathbf{d}}$ that retreats from singularities along $\mathbf{d}$, a centering term that pulls each joint toward the middle $\mathbf{q}_{\mathrm{mid}}$ of its range, and a cone term gated by $g(\theta)$ that activates only as the tool axis approaches the $\theta_{\max}$ boundary. \rst{Concretely, the directional manipulability is $w_{\mathbf{d}}(\mathbf{q})=\big(\mathbf{d}^{\top}(\mathbf{J}_{p}\mathbf{J}_{p}^{\top}+\lambda^{2}\mathbf{I})^{-1}\mathbf{d}\big)^{-1/2}$ with $\lambda=10^{-3}$, and the gate is $g(\theta)=\mathrm{clip}\!\big((\theta-\theta_{\max})/\theta_{\mathrm{m}},0,1\big)$ with margin $\theta_{\mathrm{m}}=8^{\circ}$. The damped pseudo-inverse $\mathbf{J}^{+}$ uses Nakamura--Hanafusa adaptive damping ($\lambda_{0}=0.05$, engaged as $\sigma_{\min}(\mathbf{J}_{p})$ drops below $\sigma_{\mathrm{thr}}=0.05$).} Notably, all subsequent controllers share this task decomposition, differing only in the strategy used to generate the secondary command $\boldsymbol{\xi}$.
...
\rst{Although the initialization precedes execution in time, we present the online controller first, as the null-space controller and its expert-iteration improvement are the core of the framework, and then return to the pre-execution initialization in Sec.~\ref{sec:ranking}.}
...
\subsection{\xinyi{Student Policy:} RL-based Null-Space Controller}\rst{\label{sec:student}}
...
\end{equation}
the \rst{actual} per-step \rst{end-effector} advance along $\mathbf{d}$ normalized by its maximum attainable value \rst{$v\,\Delta t$ (with $w_{p}{=}1$)}. No terminal penalty is imposed during the training process, and any violation directly terminates the episode. \rst{Because the task term commands a constant forward velocity $v\mathbf{d}$, each feasible step earns a reward close to $w_{p}$; the return is thus approximately proportional to the number of feasible steps, so maximizing the discounted return ($\gamma{=}0.99$) is equivalent to maximizing the executable path length, up to the episode cap of $10{,}000$ steps. The actor and critic are three-hidden-layer ReLU MLPs, and the policy is a tanh-squashed Gaussian trained with PPO (GAE $\lambda{=}0.95$, clip $0.2$, learning rate $3\times10^{-4}$).}
...
We therefore improve the policy through iterated distillation. Rather than requiring the RL policy to discover the behavior on its own, we take the hybrid controller as an expert and distill it into a single student policy of the same architecture as the RL policy; \sout{the improved student then constitutes a stronger expert for the next round, closing the loop} \rst{the expert is held fixed---its interior branch is the original RL policy $\hat{\pi}_{0}$ and its shell is the classical controller---while the student is corrected toward it over successive rounds of dataset aggregation, so that the single network progressively internalizes the near-boundary behavior on the very states it visits}.

The learning target is a fixed expert policy $\pi^{*}$, constructed once around the \sout{current student $\hat{\pi}$}\rst{PPO-trained policy $\hat{\pi}_{0}$} as the hybrid null-space controller described above. When queried at a state, the expert returns \sout{the student's own action}\rst{$\hat{\pi}_{0}$'s action} in the interior region, the classical controller's action in the boundary shell, and a linear blend of the two within a narrow band around the switching threshold; the blend removes the switching discontinuity that a smooth student network cannot represent. This construction, summarized in Algorithm~\ref{alg:expert}, requires only the two existing controllers \sout{and introduces no search, learned model, or additional hyperparameter} \rst{and introduces no search, learned model, or online optimization; the only added quantity is the narrow blending band $b$}.

The iterated distillation then proceeds as in Algorithm~\ref{alg:exit}. In each round, the current student is rolled out in closed loop on the training tasks to collect the states it visits; the fixed expert relabels these states, and the labels are aggregated with those of previous rounds in the manner of DAgger~\cite{ross2011reduction}, so that the student is corrected exactly on its own visitation distribution. The student is then refit to the aggregated labels by supervised regression, warm-started from its predecessor to preserve fidelity. \sout{Because the expert's interior branch is the student itself, an improved student yields a stronger expert, and the two improve together} \rst{The aggregation, not any change in the expert, drives the improvement: each round supplies the boundary states the current student still under-visits, so the single network converges onto the fixed hybrid target where a one-shot clone would suffer distribution shift}.

The loop is terminated by an improvement criterion: after each round we measure the fraction of tasks on which reinstating the runtime switch still lengthens the rollout beyond the student alone. This fraction \sout{decreases monotonically across rounds, indicating that the student progressively internalizes the boundary behavior; when it stabilizes, the expert no longer surpasses the student and the improvement operator is exhausted} \rst{drops sharply over the first few rounds and then fluctuates within a narrow plateau, indicating that the student has internalized most of the boundary behavior; once it stabilizes, the residual benefit of reinstating the switch is marginal and the loop is stopped}. The final policy $\pi_{\mathrm{D}}$ is taken as the \sout{parameter average of the last two students in the lineage~\cite{wortsman2022model}} \rst{converged student at the plateau}. The distilled policy then serves as the learning-based controller within the deployed framework.}

\begin{algorithm}[!htbp]
{\color{magenta}
\caption{Iterated distillation of the fixed hybrid expert.}
\label{alg:exit}
\KwIn{PPO-trained apprentice $\hat{\pi}_{0}$ (Sec.~\ref{sec:student}); rounds $N$}
\KwOut{distilled policy $\pi_{\mathrm{D}}$}
$\pi^{*} \leftarrow \mathrm{BuildExpert}(\hat{\pi}_{0})$ \tcp*{fixed expert, Alg.~\ref{alg:expert}}
$\mathcal{D} \leftarrow \emptyset$\;
\For{$i \leftarrow 1$ \KwTo $N$}{
  $\mathcal{S}_{i} \leftarrow$ states visited by $\hat{\pi}_{i-1}$ in closed loop on training tasks\;
  $\mathcal{D}_{i} \leftarrow \{(\mathbf{s},\, \pi^{*}(\mathbf{s})) \mid \mathbf{s} \in \mathcal{S}_{i}\}$ \tcp*{fixed-expert labels}
  ...
  \lIf{the runtime switch no longer improves on $\hat{\pi}_{i}$}{\textbf{break}}
}
\Return $\pi_{\mathrm{D}} \leftarrow \hat{\pi}_{N}$ \tcp*{converged student}\;
}
\end{algorithm}
...
\label{alg:expert}
\KwIn{apprentice $\hat{\pi}_{0}$; threshold $\tau$; band $b$}
\KwOut{expert action $\pi^{*}(\mathbf{s})$}
$\rho \leftarrow$ normalized joint-limit distance of $\mathbf{s}$, Eq.~\eqref{eq:rho}\;
\lIf{$\rho < \tau - b$}{\Return $\hat{\pi}_{0}(\mathbf{s})$ \tcp*[f]{interior: the apprentice}}
...
\Return $(1-\lambda)\,\hat{\pi}_{0}(\mathbf{s}) + \lambda\,\mathbf{a}_{\mathrm{cls}}(\mathbf{s})$ \tcp*{band}
}
\end{algorithm}
...
\rev{\label{sec:exp}}
All experiments were conducted \sout{using} \rst{in kinematic simulation with} the \textit{One} robotic system\footnote{https://github.com/wanweiwei07/one}, on the 7-DoF Franka Research 3 (FR3) \rst{model} with a pen-shaped tool attached to the end-effector. \rst{Hardware validation is left to future work.} \rev{We sample $100{,}000$ collision-free straight-line path-following tasks within the FR3 work range, each specified by a starting position $\mathbf{p}_0$, a desired line direction $\mathbf{d}$, and a task-plane normal $\mathbf{n}$. The tasks are split once into a training set and a test set: we reserve $5{,}000$ tasks as a held-out test set that is frozen and never observed by any training component, and we run the self-motion-manifold labeling procedure on the remaining tasks and retain the $20{,}000$ qualified ones---those admitting at least one branch of sufficient reachable path length---as the training set. This single labeling pass supplies all three learned components: the qualified branches whose length lies within the top region serve as the diffusion training targets, the full set of candidates with their measured lengths serves as the ranking training data, and all enumerated branch configurations serve as the initial states for reinforcement learning. The reference length is computed only on the test set for evaluation and never enters training.}

Unless otherwise specified, experiments use the default setting: \rev{(1)} a diffusion-based initial configuration generator using DDIM with 50 denoising steps and CFG scale $w=1.5$; \rev{(2)} classical secondary-objective weights $k_{\mu},k_{\mathrm{jl}},k_{\theta}$ of $0.8,0.4,0.2$; \rev{(3)} \sout{hybrid switching thresholds $\tau_{\mathrm{enter}},\tau_{\mathrm{exit}}$ of $0.985,0.96$;} \rst{a single memoryless switching threshold $\tau_{\mathrm{enter}}=\tau_{\mathrm{exit}}=0.975$ for the hybrid expert, which the distillation requires (a hysteresis gap would make the expert non-Markovian; see Sec.~\ref{sec:coordination} and Table~\ref{tab:ablate_switch});} \rev{and (4) ranked initialization over a batch of diffusion candidates, from which the learned scorer selects the executed seed}.

\rev{For each test task, we generate candidate configurations following the same enumeration procedure and roll out every candidate under the classical controller. We denote the longest rollout length as the \emph{reference length} ($\ell^{\mathrm{ref}}$) and use it for the subsequent percentage comparison. As the reference is attained by a specific controller rather than a task-intrinsic bound, a configuration may exceed it, in which case the reported ratio surpasses $100\%$. \rst{This is thus an empirical, enumeration-classical reference, not an upper bound on the achievable length; percentages should be read against it accordingly, and the reported ratios are means of per-task ratios.}}
\xinyitodo{待补：(1) 除 mean-of-ratios 外，报告 median/IQR、聚合比 $\sum_i \ell_i/\sum_i \ell_i^{\mathrm{ref}}$ 与 absolute regret $\ell_i^{\mathrm{ref}}-\ell_i$，以抵消小分母导致的 ratio 爆炸（max 达 600\%+）；(2) 对被排除的 251 个 zero-reference 任务，单独报告各方法的 absolute length 与成功率，说明零参考的成因（几何不可行 / 枚举失败 / 控制器失败）。}

\subsection{Results Analysis}
\subsubsection{Evaluation Set Analysis}
We partition the \rev{test} set into difficulty levels by the absolute reference length $\ell^{\mathrm{ref}}$: Easy ($\ell^{\mathrm{ref}}\ge 0.80$\,m, \sout{\rev{$n{=}0$}}~\rst{$n{=}1393$}), Medium ($0.45\le\ell^{\mathrm{ref}}<0.80$\,m, \sout{\rev{$n{=}0$}}~\rst{$n{=}1729$}), and Difficult ($\ell^{\mathrm{ref}}<0.45$\,m, \sout{\rev{$n{=}0$}}~\rst{$n{=}1627$}). \rst{The three buckets total $4{,}749$ evaluable tasks; the remaining $251$ of the $5{,}000$ held-out tasks admit no valid rollout ($\ell^{\mathrm{ref}}{=}0$) and are excluded from the ratio statistics.} \sout{Intuitively, harder tasks exhibit a shorter best achievable path, reflected in a smaller reference length.} \rst{These groups characterize the length of the empirical reference horizon rather than algorithmic difficulty: a short reference reflects a short feasible horizon, and in fact the relative ratios are lowest on the long-horizon (Easy) group.} Fig. \ref{fig:eval_dist} (a) shows the distribution of $\ell^{\mathrm{ref}}$ \rev{over the test set}.
...
\midrule
\multirow{5}{*}{\shortstack{All\\{\scriptsize \rev{($n{=}4749$)}}}}
& \rev{Classical}                  & \rev{0.40 / 0.21 / 0.00 / 1.23} & \rev{72.21 / 32.61 / 0.00 / 636.55} \\
& RL                        & \rev{0.50 / 0.29 / 0.00 / 1.59} & \rev{83.25 / 33.01 / 0.00 / 636.55} \\
& $\pi_{\mathrm{D}}$ (distilled)      & \rev{0.51 / 0.29 / 0.00 / 1.59} & \rev{84.89 / 31.94 / 0.00 / 636.55} \\
& Hybrid                    & \rev{0.52 / 0.29 / 0.00 / 1.59} & \rev{86.61 / 30.77 / 0.00 / 636.55} \\
& Hybrid($\pi_{\mathrm{D}}$)          & \rev{0.52 / 0.28 / 0.00 / 1.59} & \rev{86.27 / 31.03 / 0.00 / 636.55} \\
\midrule
\multirow{5}{*}{\shortstack{Easy\\{\scriptsize \rev{($n{=}1393$)}}}}
& \rev{Classical}                  & \rev{0.49 / 0.27 / 0.00 / 1.23} & \rev{49.98 / 29.39 / 0.00 / 112.04} \\
& RL                        & \rev{0.72 / 0.34 / 0.00 / 1.59} & \rev{71.38 / 33.22 / 0.00 / 137.07} \\
& $\pi_{\mathrm{D}}$ (distilled)      & \rev{0.74 / 0.33 / 0.00 / 1.59} & \rev{73.51 / 32.09 / 0.00 / 119.19} \\
& Hybrid                    & \rev{0.77 / 0.31 / 0.00 / 1.59} & \rev{76.59 / 30.05 / 0.00 / 138.21} \\
& Hybrid($\pi_{\mathrm{D}}$)          & \rev{0.76 / 0.32 / 0.00 / 1.59} & \rev{75.64 / 30.37 / 0.00 / 119.19} \\
\midrule
\multirow{5}{*}{\shortstack{Medium\\{\scriptsize \rev{($n{=}1729$)}}}}
& \rev{Classical}                  & \rev{0.46 / 0.16 / 0.00 / 1.15} & \rev{75.33 / 23.89 / 0.00 / 180.07} \\
& RL                        & \rev{0.53 / 0.18 / 0.00 / 1.33} & \rev{86.19 / 25.81 / 0.00 / 194.01} \\
& $\pi_{\mathrm{D}}$ (distilled)      & \rev{0.54 / 0.17 / 0.00 / 1.32} & \rev{87.95 / 24.47 / 0.00 / 208.81} \\
& Hybrid                    & \rev{0.55 / 0.17 / 0.00 / 1.35} & \rev{89.24 / 23.52 / 0.00 / 206.33} \\
& Hybrid($\pi_{\mathrm{D}}$)          & \rev{0.55 / 0.17 / 0.00 / 1.35} & \rev{88.95 / 23.39 / 0.00 / 206.35} \\
\midrule
\multirow{5}{*}{\shortstack{Difficult\\{\scriptsize \rev{($n{=}1627$)}}}}
& \rev{Classical}                  & \rev{0.27 / 0.10 / 0.00 / 1.21} & \rev{87.94 / 32.81 / 0.00 / 636.55} \\
& RL                        & \rev{0.28 / 0.12 / 0.00 / 1.21} & \rev{90.30 / 36.71 / 0.00 / 636.55} \\
& $\pi_{\mathrm{D}}$ (distilled)      & \rev{0.28 / 0.11 / 0.00 / 1.33} & \rev{91.39 / 36.01 / 0.00 / 636.55} \\
& Hybrid                    & \rev{0.28 / 0.11 / 0.00 / 1.21} & \rev{92.39 / 35.71 / 0.00 / 636.55} \\
& Hybrid($\pi_{\mathrm{D}}$)          & \rev{0.28 / 0.12 / 0.00 / 1.33} & \rev{92.52 / 36.00 / 0.00 / 636.55} \\
\bottomrule
\end{tabular}
\begin{tablenotes}
...
& \shortstack{Ratio to Reference ($\%$)\\ {\scriptsize Mean / Std / Min / Max}} \\
\midrule
\multirow{3}{*}{\shortstack{All\\{\scriptsize \rev{($n{=}4749$)}}}}
& First IK-valid  & \rev{0.51 / 0.29 / 0.00 / 1.59} & \rev{83.71 / 29.83 / 0.00 / 457.71} \\
& Ranked & \rev{0.56 / 0.29 / 0.00 / 1.60} & \rev{91.21 / 27.50 / 0.00 / 585.53} \\
& \textit{Best-of-batch (oracle)} & \rev{\textit{0.63 / 0.30 / 0.06 / 1.60}} & \rev{\textit{101.18 / 23.73 / 17.03 / 585.53}} \\
\midrule
\multirow{2}{*}{\shortstack{Easy\\{\scriptsize \rev{($n{=}1393$)}}}}
& First IK-valid  & \rev{0.74 / 0.33 / 0.00 / 1.59} & \rev{73.33 / 32.33 / 0.00 / 119.19} \\
& Ranked & \rev{0.86 / 0.28 / 0.00 / 1.60} & \rev{84.80 / 25.27 / 0.00 / 155.45} \\
\midrule
\multirow{2}{*}{\shortstack{Medium\\{\scriptsize \rev{($n{=}1729$)}}}}
& First IK-valid  & \rev{0.54 / 0.17 / 0.00 / 1.32} & \rev{87.61 / 24.93 / 0.00 / 208.81} \\
& Ranked & \rev{0.58 / 0.15 / 0.00 / 1.44} & \rev{93.95 / 18.72 / 0.00 / 222.55} \\
\midrule
\multirow{2}{*}{\shortstack{Difficult\\{\scriptsize \rev{($n{=}1627$)}}}}
& First IK-valid  & \rev{0.27 / 0.11 / 0.00 / 1.33} & \rev{88.45 / 30.19 / 0.00 / 457.71} \\
& Ranked & \rev{0.29 / 0.12 / 0.00 / 1.54} & \rev{93.80 / 35.19 / 0.00 / 585.53} \\
\bottomrule
\end{tabular}
\begin{tablenotes}
    \item[Note 1] \rev{Best-of-$K$ oracle prefixes: $K{=}2$: $86.1\%$, $K{=}4$: $91.0\%$, $K{=}8$: $94.2\%$, rising steeply toward the best-of-batch \sout{bound} \rst{oracle}, so the sampled candidates \sout{are \emph{not} exchangeable} \rst{are \emph{not} equal in realized downstream quality} and selection is necessary.}
    \item[Note 2] \rev{Selection cost: $<\!0.2$\,s/task (batched DDIM $+$ Newton projections $+$ one scorer forward pass). No rollout is performed at deployment.}
\end{tablenotes}
\end{threeparttable}}
\end{table*}
...
\midrule
\rev{First IK-valid (no selection)}                 & \rev{83.71} \\
\midrule
\rev{Pointwise regression}                          & \rev{88.12} \\
\rev{Contrastive~\cite{khosla2020supervised}}       & \rev{88.21} \\
\rev{ListNet (listwise)~\cite{cao2007learning}}     & \rev{88.21} \\
\rev{RankNet (pairwise)~\cite{burges2005learning}}  & \rev{89.81} \\
\rev{ListMLE (listwise)~\cite{xia2008listwise}}     & \rev{90.37} \\
\rev{LambdaRank (adopted)~\cite{burges2010ranknet}} & \rev{\limebox{90.61}} \\
\midrule
\rev{\textit{Best-of-batch (oracle)}}               & \rev{\textit{101.18}} \\
\bottomrule
\end{tabular}
\begin{tablenotes}
    \item[Note 1] \rev{All rows share the same candidate batch, features, and the deployed single-network controller $\pi_{\mathrm{D}}$, and use a $3$-model ensemble; they differ only in the ranking objective. The choice of objective has little effect (all within a $2.5\%$ band): the seed need not be optimal, only within the top region. We adopt LambdaRank; the deployed scorer uses a $10$-model ensemble of it, reaching $91.2\%$ (Table~\ref{tab:ranked}).}
\end{tablenotes}
\end{threeparttable}}
\end{table}
...
---        & \pinkbox{1.0} & ---        & 0.30 / 0.17 / 0.01 / 1.20 & \pinkbox{56.82} / 29.69 / 0.73 / 352.50 \\
---        & ---        & 1.0 & 0.30 / 0.16 / 0.01 / 1.20 & 57.23 / 28.75 / 0.71 / 239.50 \\
\midrule
...
0.8 & 0.4 & 0.2 & 0.33 / 0.17 / 0.01 / 1.20 & \limebox{60.74} / 29.16 / 0.73 / 225.40 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
...
3.0 & \pinkbox{77.8} & 0.55 / 0.30 / 0.00 / 1.60 & 89.66 / 29.60 / 0.00 / 577.60 \\
\bottomrule
\end{tabular}
\end{threeparttable}
\end{table*}
...
Table~\ref{tab:ablate_steps} examines the influence of the DDIM sampling steps on seed quality and sampling time. The best-of-batch length is essentially flat across the range $10$--$100$ ($92.3$--$93.5\%$), so seed quality is retained within a few denoising steps, consistent with quality being largely a function of the SMM branch. The batched per-task sampling time (column $t$, amortized over the batch) grows roughly linearly with the step count but stays well below a millisecond, so the number of steps is not a practical cost driver. We adopt $50$ as the default.
...
& \shortstack{Ratio to Reference ($\%$)\\ {\scriptsize Mean / Std / Min / Max}} \\
\midrule
\phantom{0}10  & 0.10 & 0.57 / 0.30 / 0.00 / 1.60 & 92.32 / 26.63 / 0.00 / 460.60 \\
\phantom{0}20  & 0.13 & 0.57 / 0.30 / 0.00 / 1.60 & 92.44 / 26.02 / 0.00 / 460.70 \\
\phantom{0}50  & \limebox{0.19} & \limebox{0.58} / 0.30 / 0.00 / 1.60 & \limebox{92.91} / 25.34 / 0.00 / 460.70 \\
100 & 0.38 & 0.58 / 0.30 / 0.00 / 1.60 & 93.47 / 25.84 / 0.00 / 503.90 \\
\bottomrule
\end{tabular}
\end{threeparttable}
\end{table*}
...
$(0.98,\,0.94)$ & \pinkbox{0.32} & 0.52 / 0.29 / 0.00 / 1.60 & 86.26 / 30.78 / 0.00 / 658.70 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
...
Table~\ref{tab:ablate_exit} traces the \sout{expert iteration}\rst{iterated distillation} round by round. Round $0$ is the PPO policy; each subsequent round \sout{rebuilds the expert around the current student and distills it}\rst{distills the same fixed hybrid expert on the states the current student visits}. \sout{Two quantities are reported: the ratio to reference of the standalone student, and the fraction of tasks on which reinstating the runtime switch still lengthens the rollout beyond the student alone, which serves as the improvement criterion.}\rst{For each round we report the standalone student's ratio to reference, the switch-benefit fraction (the share of tasks on which reinstating the runtime switch still lengthens the rollout), and the expert gain $\Delta_i=\mathbb{E}\!\left[\mathcal{L}(\mathrm{Hybrid}(\hat{\pi}_i))-\mathcal{L}(\hat{\pi}_i)\right]$, the mean path length by which the switch would extend the rollout.} The standalone student \sout{improves monotonically while the switch-benefit fraction decreases, indicating that the boundary behavior is progressively internalized}\rst{improves over the first few rounds and then fluctuates within a narrow plateau, while the switch-benefit fraction drops from $16.7\%$ to $8.3\%$ and the expert gain shrinks from $21.7$ to about $9$\,mm, indicating that most of the boundary behavior has been internalized}; once \sout{the fraction stabilizes}\rst{these stabilize}, the \sout{expert no longer surpasses the student and the loop is terminated}\rst{residual benefit of reinstating the runtime switch becomes marginal and the loop is terminated}.}

\rev{Table~\ref{tab:ablate_ablation} isolates the role of each ingredient under a matched, leak-safe protocol in which every variant is warm-started from $\hat{\pi}_{0}$ and shares the same switching threshold, blending band, and per-round task budget. Continuing PPO from $\hat{\pi}_{0}$ for a budget matched to the distillation's collected states does not internalize the behavior: the standalone ratio does not rise and the switch-benefit fraction even increases, confirming that the obstacle is exploration and retention rather than training budget. A one-shot clone of the hybrid expert barely improves on the PPO policy, because the student drifts off the expert's state distribution; the DAgger aggregation, which supplies exactly the boundary states the student under-visits, is what closes the gap. Finally, rebuilding the expert around the improved student every round---the literal expert-iteration loop---does not help and is in fact slightly worse and non-monotone: since the rebuilt expert's interior branch is itself a distilled approximation, its labels accumulate error across rounds, whereas the fixed hybrid target is stable. We therefore hold the expert fixed and iterate only the aggregation distribution.}

\begin{table}[!htbp]
\centering
\rev{
\caption{\rev{Convergence of Iterated Distillation over Rounds.}}
...
$6$                & $84.89$ & $8.3$  & $9.1$  \\
\midrule
$\pi_{\mathrm{D}}$ (round $6$) & \limebox{$84.89$} & $8.3$ & $9.1$ \\
\bottomrule
\end{tabular}
\begin{tablenotes}
...
\centering
\rev{
\caption{\rev{Ablation of the Iterated-Distillation Ingredients.}}
...
\;+ continued PPO (budget-matched)                   & $82.58$ \\
