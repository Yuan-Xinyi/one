...
\item We construct a hybrid null-space expert that combines the long-horizon learned policy in the well-sampled interior with the reliable classical controller near the joint limits, and \rst{distill this fixed expert into a single learning-based policy through repeated on-policy dataset aggregation, querying the same hybrid expert on the states visited by the evolving student}.
\item We introduce a diffusion prior with a learned scorer that selects a favorable \rst{component of the feasible configuration set} as the initial configuration, and analyze the contribution of initialization, control, switching, and \rst{iterated distillation} on a large held-out task set.
\end{itemize}}
...
\rst{Although the initialization precedes execution in time, we present the online controller first, as the null-space controller and its iterated-distillation improvement are the core of the framework, and then return to the pre-execution initialization in Sec.~\ref{sec:ranking}.}
...
\rev{\subsection{\xinyi{Iterated Distillation of the Hybrid Expert}}\label{sec:distill}
...
\rev{\subsubsection{Iterated Distillation: Convergence and Ablation}
