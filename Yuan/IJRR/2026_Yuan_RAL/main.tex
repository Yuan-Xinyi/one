...
sound: we show that achieved strokes exceed such a reference. We instead bound
each task from above by marching along its own path and testing whether any
admissible configuration exists at each sample, which depends on the kinematics
alone and cannot be exceeded by any resolution law. Against this bound we report,
...
envelope, and that on curved paths the degradation is governed by the total
angle the path turns through rather than by its curvature. Because a query costs
...
\end{equation}
where $\mathbf{J}^{+}$ is the damped pseudo-inverse and $(\mathbf{I}-\mathbf{J}^{+}\mathbf{J})$ projects the secondary command $\boldsymbol{\xi}$ onto the null space. \new{In implementation the projected term is written in an exact null-space basis (Section~\ref{sec:traj}), so the damping affects only the task term.} Within this null space, several secondary objectives can be pursued without disturbing the end-effector position:
\begin{equation}
\label{eq:secondary}
...
of Section~\ref{sec:traj}). Throughout this work the
secondary command is bounded through the parameterization
$\boldsymbol{\xi} = a_{\max}\,\mathbf{B}(\mathbf{q})\,\mathbf{a}$ with
$\mathbf{a} \in \mathcal{A} := [-1,1]^{m}$, where $m$ is the dimension of the
null space left by the position task ($m=4$ for the two 7-DoF arms, $m=3$ for
Cobotta) and $\mathbf{B}(\mathbf{q})$ is
the task-aligned null-space basis constructed in Section~\ref{sec:traj}. Under
this parameterization the decomposition of Eq.~(\ref{eq:nullspace}) is
control-affine,
\begin{equation}
\label{eq:affine}
\dot{\mathbf{q}} \;=\; \mathbf{f}(\mathbf{q}) \;+\; \mathbf{G}(\mathbf{q})\,\mathbf{a},
\qquad
\mathbf{f} := \mathbf{J}^{+}\dot{\mathbf{x}},\quad
...
control period gives $\beta = (1-\gamma)/\Delta t = 0.2\,\mathrm{s}^{-1}$). On
...
maximum-exit-time problem. The two decision variables of the constrained
program above reappear as the two stages of the framework: the selection stage
chooses $\mathbf{q}_{0}^{*}$ to maximize $V$ across the branches of the
self-motion manifold, and the trajectory stage learns a feedback policy that
...
in $\mathbf{a}$ by the definition of the null-space decomposition. Second, the
advance rate $r$ does not depend on $\mathbf{a}$: $\mathbf{B}$ is an exact
basis of the null space of the position Jacobian (identified by SVD in double
precision; the damping of $\mathbf{J}^{+}$ enters only the particular
solution), so the secondary command cannot alter the instantaneous advance.
...
absorbed, as $\Phi(\mathbf{q})$, the two laws read, in the shared basis,
\begin{equation}
\label{eq:cl-vs-opt}
...
classical law is thereby a member of the same family of
switching-function controllers, with $\Phi$ in place of $V$ and the magnitude
retained --- a characterization confirmed empirically in
Table~\ref{tab:action_space}, where the deployed classical law uses a median
$14\%$ of its command authority and the learned policy $100\%$.
...
design choice left is the secondary command $\boldsymbol{\xi}$.}

\new{The classical choice is to set $\boldsymbol{\xi}$ by gradient ascent on the
...
\end{equation}
the per-step advance along $\mathbf{d}$ normalized by its maximum attainable value\new{; $w_{p}=1$ throughout}. With no terminal penalty, any violation simply ends the episode, so the return reduces to the survival time, and the policy is rewarded solely for sustaining the motion.
...
pointwise-reachable length realized by a single continuous stroke (\%); $100\%$ is a strict
upper bound. Best results in \textbf{bold}.}
...
\midrule
Continuous in $[-1,1]^{4}$      & -- & -- & --- & --- \\
Projected onto nearest vertex   & -- & -- & -- & -- \\
Trained over the $2^{4}$ vertices & -- & -- & -- & -- \\
\bottomrule
\end{tabular}
\end{table}
...
and the sweep in Appendix~\ref{app:sweeps}, run on all three, shows that the
optimum does not move appreciably. The second is whether switching leaves a
...
\color{red}
    ...
    \label{fig:ratelimit}
\end{figure}
\new{The threshold sweep (Appendix~\ref{app:sweeps}) shows the hysteresis gap reducing the mean number of switches per stroke from $1.43$ to $0.44$ at a cost of $0.1$\,pp in length; $(0.98,0.94)$ is therefore used throughout.}
...
reachability, measured it against a bound that depends on the kinematics alone
and therefore cannot be exceeded, and proposed a two-stage framework to close
part of the gap. The first stage treats the choice of initial configuration as a
choice among the disconnected branches of the self-motion manifold: candidates
...
\begin{tablenotes}
    ...
    \item[Note 2] \new{To be regenerated: measured on one manipulator against the
    earlier best-of-$K$ reference length.}
\end{tablenotes}
\end{threeparttable}
\end{table*}
