...
if [ ! -f Yuan/IJRR/runs/rl_vertex_line_bscale_30M/agent.pt ]; then
  python -m Yuan.IJRR.stage2_traj.train \
    --config Yuan/IJRR/stage2_traj/config_vertex_line_bscale.yaml \
    --out-dir Yuan/IJRR/runs/rl_vertex_line_bscale_30M \
    > "$FU/train_bscale.log" 2>&1
fi
python "$SC/bscale_eval.py" > "$FU/bscale_eval.log" 2>&1
touch "$FU/bscale_all.done"
