...
TI = int(sys.argv[1]); RAD = float(sys.argv[2]) if len(sys.argv) > 2 else None
...
sub = {'q0': torch.tensor(tz['q0_seed'][TI][None], dtype=dt_t, device=dev),
       ...
       'kappa': torch.tensor([1.0/RAD] if RAD else tz['kappa'][TI][None], dtype=dt_t, device=dev)}
...
nt = nt/nt.norm()
kap = torch.tensor([1.0/RAD] if RAD else tz["kappa"][TI][None], dtype=dt_t, device=dev)
s_ring = torch.linspace(0, float(2*np.pi*RAD) if RAD else float((2*np.pi*RAD if RAD else 2*np.pi/abs(tz["kappa"][TI]))), 145,
