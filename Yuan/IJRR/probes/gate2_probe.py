...
def run(afn,tag):
    ...
    return r
with torch.no_grad():
    r1=run(hl.make_vlook(model,env,ag),'vlook (1-step gate)')
r2=run(vlook2,'vlook2 (viability gate)')
np.savez('/home/lqin/one/Yuan/IJRR/runs/paper_fill/fam_unify/gate2_probe.npz', g1=r1, g2=r2)
d=r2-r1
print(f'per-task delta: mean {d.mean():+.4f}  improved {(d>0.01).mean()*100:.1f}%  hurt {(d<-0.01).mean()*100:.1f}%')
