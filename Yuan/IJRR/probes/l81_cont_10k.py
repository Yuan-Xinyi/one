...
ag.load_state_dict(torch.load(REPO/'Yuan/IJRR/runs/rl_cont_sqent_30M/agent.pt', map_location=dev)); ag.eval()
verts=torch.tensor(list(itertools.product([-1.,0.,1.],repeat=env.act_dim)),dtype=torch.float32,device=dev)
...
def fn(e,done):
    ...
    v=torch.cat([ag.critic(hl._obs_of(e,qn[i:i+chunk],de[i:i+chunk],ne[i:i+chunk],ae[i:i+chunk])).squeeze(-1) for i in range(0,Bn*K,chunk)]).reshape(Bn,K)
...
np.savez('/home/lqin/one/Yuan/IJRR/runs/paper_fill/fam_unify/l81_cont_10k.npz', prog=out)
...
cv=np.load('/home/lqin/one/Yuan/IJRR/runs/paper_fill/fam_unify/contcritic_vlook_10k.npz')['prog']
...
for v in (out,cv): ref=np.maximum(ref,v)
def stat(v):
    rt=v/np.maximum(ref,1e-9)
    return f'{v.mean():.4f}  {rt.mean()*100:.1f} / {np.percentile(rt,10)*100:.1f}  t27 {v[27]:.3f}'
print('cont-critic V16  (10k):', stat(cv))
print('cont-critic L81  (10k):', stat(out))
