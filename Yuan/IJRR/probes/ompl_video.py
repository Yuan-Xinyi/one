#!/usr/bin/env python3
"""one-viewer clip of the planner against the framework on one figure.

Green is OMPL's plan, crimson is the learned framework. Each starts from the
start IT chose -- the planner samples its own, the framework's selection stage
picks from its candidate pool -- because that is what the reported sizes mean.
Each lays its own trace, so where a colour stops is where that method ran out.
...
PEN_LEN = 0.10
COL = {'a': (0.10, 0.60, 0.45), 'b': (0.86, 0.16, 0.30)}   # a=OMPL, b=framework
ALPHA = 0.40
CH = 6
...
for tag in ('a', 'b'):
    ...
    tip = d[f'tip_{tag}']
    lift = np.float32([0, 0, 0.003 if tag == 'b' else 0.007])
    seg, fr_, last = [], [], tip[0]
