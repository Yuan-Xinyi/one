    ...
    for base in range(0, len(task_ids), B):
        ...
        for t in range(env.max_steps):
            ...
            if TEACHER is not None:
                act_t = TEACHER(env, env.done_persistent)
                act = ((act_t > 0).float()
                       * torch.tensor([8., 4., 2., 1.], device=dev)
                       ).sum(-1).long()
...
def ladder_eval(critic, tag):
    ...
    if not mm:
        say(f'[polit] ladder FAILED for {tag}: {r.stdout[-400:]} '
            f'{r.stderr[-400:]}')
        return None, None
    prog = float(mm.group(1))
    return prog / 0.3353, prog                 # classical mean on this ladder


say(f"[polit] PHASE 2: beam teacher. Baselines on the 1024-task ladder: "
    f"vlook(PPO critic) 0.5678 m, vbeam8x3 teacher itself 0.5872 m")
...
def set_teacher(crit):
    global TEACHER
    _W.critic = crit
    TEACHER = hl.make_vbeam(model, env, _W, 8, 3)


best = (0, 0.5678 / 0.3353)
N_TASKS, ROUNDS = 4096, 3
for rnd in range(1, ROUNDS + 1):
    t0 = time.time()
    ids = np.arange(COLLECT_BASE + 6144 + (rnd - 1) * N_TASKS,
                    COLLECT_BASE + 6144 + rnd * N_TASKS)
    set_teacher(critic)
    OBS, RET = collect(critic, ids, eps_levels=(0.0, 0.1),
                       eps_frac=(0.7, 0.3))
