...
FR = OUT / 'stc_prog_frames'; FR.mkdir(parents=True, exist_ok=True)
PEN_LEN, CH, STEPF = 0.10, 10, 0.012
LAYCOL = [(0.86, 0.16, 0.30), (0.93, 0.55, 0.10),
          (0.20, 0.65, 0.30), (0.20, 0.40, 0.85)]

d = np.load(OUT / 'stc_prog_pack.npz', allow_pickle=False)
...
for i in range(1, len(tip)):
    if np.linalg.norm(tip[i] - last) > 0.006:
        ...
        cols.append(LAYCOL[min(int(np.searchsorted(STAGES, arc[i], 'right') - 1), len(LAYCOL) - 1)])
        last = tip[i]
CHUNKS = []
i = 0
while i < len(seg):
...
def tick(_dt):
    ...
    k = min(t, K - 1)
    arm.fk(Q[k])
    lay = min(int(np.searchsorted(STAGES, arc[k], 'right') - 1),
              len(LAYCOL) - 1)
    pen.rgb = LAYCOL[lay]
