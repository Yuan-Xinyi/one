"""Roll the hollow one-stroke letters (mode=word) or the Euler one-stroke
mark (mode=mark) at the largest size the placement search admits."""
import sys, time
...
def main(mode):
    if mode == 'word':
        strokes, _ = wrs2_lib.hollow_word()
        tag = 'wrs_hollow'
    elif mode == 'wordone':
        Q, _ = wrs2_lib.word_one_stroke()
        strokes = [('word', Q)]
        tag = 'wrs_onestroke'
    elif mode == 'logo2':
        import wrslogo2_lib
        Q, _, _ = wrslogo2_lib.one_stroke()
        strokes = [('mark', Q)]
        tag = 'wrslogo2'
    else:
        Q, _, _ = wrs2_lib.mark_one_stroke()
        strokes = [('mark', Q)]
        tag = 'wrslogo_euler'
    all2 = np.concatenate([wrs_lib.densify(p, 0.02) for _, p in strokes])
    ...
    print(f'{tag}: pointwise max {pw.max()*100:.0f} cm', flush=True)
        ...
        for c in np.argsort(-pw)[:4]:
            ...
            for name, p2 in strokes:
                ...
                if a[0] < need - 3 * osr.STEPW:
                    print(f'  H={H*100:.0f} {name}: '
                          f'{a[0]:.2f}/{need:.2f} FAIL', flush=True)
                    good = False; break
            ...
            for name, W, Q_, need in packs:
                tps, zxs = [], []
                for lo in range(0, len(Q_), 4096):
                    p_, R_, _, _ = env0.kin.tcp_fk_jac(
                        torch.as_tensor(Q_[lo:lo+4096], device='cuda',
                                        dtype=dt_))
                    tps.append(p_.cpu().numpy())
                    zxs.append(R_[:, :, 2].cpu().numpy())
                save[f'{name}_fig'] = W
                save[f'{name}_q'] = Q_
                save[f'{name}_tip'] = np.concatenate(tps).astype(np.float32)
                save[f'{name}_zax'] = np.concatenate(zxs).astype(np.float32)
                save[f'{name}_need'] = np.float32(need)
            np.savez_compressed(OUT / 'ompl' / f'{tag}_pack.npz', **save)
            tot = sum(need for *_, need in packs)
            print(f'SAVED {tag}: H={H*100:.0f} cm, strokes {tot:.2f} m, '
                  f'bed z={place[2]:+.2f}, place ({place[0]:+.2f},'
                  f'{place[1]:+.2f}) ({time.time()-t00:.0f}s)', flush=True)
            return
