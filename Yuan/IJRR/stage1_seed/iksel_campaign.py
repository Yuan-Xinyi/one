...
from Yuan.IJRR.stage1_seed.checkpoint import (
    build_env_from_run, load_controller_agent, load_run_config,
    ppo_config_from_run, resolve_controller_dir)
from Yuan.IJRR.stage1_seed.controller_rollout import (
    FrozenHybridController, rollout_selected_seeds)
from Yuan.IJRR.env.classical_nullspace import ClassicalNullspaceController
from Yuan.IJRR.stage1_seed.validity import check_candidate_validity
from Yuan.IJRR.stage1_seed.feature_build import _build_features
...
from Yuan.IJRR.stage1_seed.iksel_clean_pilot import (
    _table_path, _minimal_rotvec, POS_SCALE, DEDUP_RAD, N_DIRS, CONE_DEG)

T = Path('Yuan/IJRR/runs/tasks')
G = Path(os.environ.get('IKSEL_DIR', 'Yuan/IJRR/runs/iksel_final'))
CLEAN = Path('Yuan/IJRR/runs/iksel_clean_v1')
...
def _geoms(source):
    paths = {
        'train': T / 'train_geoms.npz',
        'topup': G / 'topup_tasks/candidates_K8.npz',
        'validation': T / 'validation_geoms.npz',
        'external': T / 'external_geoms.npz',
        'sealed': Path('Yuan/IJRR/runs/iksel_sealed_v3/candidates_K8.npz'),
        'eval10k': T / 'eval10k_geoms.npz',
    }
...
def stage_eval_dev(args, device):
    rep = {}
    sel = _load_sel(G / 'sel_iksel_run0.pt', device)
    for which in ('validation', 'external'):
        X, P, V = _load_pool_env(_cand(which), _labels(which), device)
        i = torch.arange(len(P), device=device)
        picked = P[i, _picks(*sel, X, V)].cpu().numpy()
        first = P[i, V.float().argmax(1)].cpu().numpy()
        ora = torch.where(V, P, torch.tensor(-1e9, device=device)
                          ).max(1).values.cpu().numpy()
        rep[which] = {
            'iksel_m': float(picked.mean()),
            'first_valid_m': float(first.mean()),
            'oracle_m': float(ora.mean()),
            'capture_pct': float((picked - first).sum() / (ora - first).sum() * 100),
        }
        r = rep[which]
        print(f"[dev {which}] iksel={r['iksel_m']:.4f} "
              f"first={r['first_valid_m']:.4f} oracle={r['oracle_m']:.4f} "
              f"cap {r['capture_pct']:.1f}%", flush=True)
