"""Branch comparison: motivation slices for 6 maximally-diverse IK starts.

Run:
    python -m Yuan.RL.v18_branch_comparison
    python -m Yuan.RL.v18_branch_comparison --seed 3 --grid 121

For a single straight-line task:
  * Enumerate IK candidates at the start TCP, filter to successful starts
    (L_start >= 0.995 * L_max under the relaxed 30 deg rollout tolerance).
  * Greedy farthest-point sampling in joint space picks 6 representatives.
    These typically fall on different IK branches; if there are not enough
    distinct branches, picks are far-apart postures within the same branch.
  * For each q, build its motivation slice (elbow swing alpha, wrist z-tilt
    beta), anchored to that q's own FK pose with position-only IK
    projection. Rollout uses theta_max = 30 deg with init pose enforcement.
  * Overlay the true 30 deg z-tilt contour computed from each grid point's
    z-axis vs the path R_tgt. Render 6 panels sharing a viridis colorbar.

Verifies whether IK branch choice dominates over intra-branch posture
choice. Compare success-region shapes across panels.
"""
...
from Yuan.RL.v18_landscape_probe import (
    ...
    SEED,
    as_tensor,
    choose_branch_pair,  # noqa: F401  (kept for symmetry with probe)
    enumerate_start_iks,
    path_length,
    rollout_lengths,
    sample_line_task,
)
from Yuan.RL.v18_motivation_probe import (
    ELBOW_SPAN,
    LINE_L_RANGE,
    ROLLOUT_THETA_MAX,
...
def main():
    ...
    out_png = OUT_DIR / f'v18_branch_comparison_seed{seed}.png'
    ...
    # the 16 anchors look visually diverse in the overlay viewer.
    q_good = q_set
    L_good = L_start
    good_idx = np.arange(q_set.shape[0])
    q_good_np = q_good.detach().cpu().numpy()
    print(f'IK candidates as pick pool: {len(good_idx)}')
    seed_idx = int(np.argmax(L_good))
    picks = farthest_point_pick(q_good_np, min(N_PICKS, len(good_idx)), seed_idx)
    ...
    for k, p in enumerate(panels):
        out_jsonl = OUT_DIR / f'v18_branch_comparison_seed{seed}_{p["label"]}.jsonl'
        alphas = p['alphas']
        betas_rad = p['betas']
        L_raw = p['L_raw']
        orient = p['orient']
