"""Diagnose P0 RL policy vs hand-tuned GPM-JL baseline on 1000 random tasks.

Same env config / termination conditions as `eval.py`, but:
  - n_holdout = 1000 (vs 200 in standard eval)
  - sample seed configurable (default 1000, disjoint from holdout_seed=42)
  - per-task joint-angle trajectory is recorded for both controllers, so
    follow-up analysis can dig into why RL underperforms on specific tasks
    without re-running the rollout

Output (under <ckpt-dir>/diag_1000/):
...
Usage:
    python -m Yuan.RL_controller.diagnose_p0_vs_baseline \\
