...
avoidance + cone attractor + q_ref attract), projected onto our 3-DOF
position-only nullspace and packaged as an action_fn compatible with
`rollout_first_episode`.
...
from Yuan.RL_controller.env.env import NSRLBatchedEnv, build_task_aligned_basis
...
class ClassicalNullspaceController:
    ...
    def __init__(self, kin,
        ...
        self.angle_attract_gain = angle_attract_gain
    ...
    def q_dot_null(self, q: torch.Tensor, u_hat: torch.Tensor,
                   n_target: torch.Tensor, q_ref: torch.Tensor) -> torch.Tensor:
        ...
        with torch.enable_grad():
            return self._compute_q_dot(q, u_hat, n_target, q_ref)

    def _compute_q_dot(self, q, u_hat, n_target, q_ref):
        ...
        # 3. Angle gradient (cone-aware)
        if self.angle_boundary_gain != 0.0 or self.angle_attract_gain != 0.0:
            ...
            if grad_cos is not None:
                ...
                angle_g = (self.angle_boundary_gain * boundary_gate
                           + self.angle_attract_gain * interior_gate)
                q_dot = q_dot + angle_g * grad_cos.detach()

        # 4. q_ref pull
        if self.k_null != 0.0:
            q_dot = q_dot + self.k_null * (q_ref - q)

        return q_dot


def cn_action_fn(controller: ClassicalNullspaceController):
    """Action closure for `rollout_first_episode`. Captures q_init per env on
    the first call after each episode reset (signaled by env.t == 0)."""
    state = {"q_ref": None}

    def _fn(env: NSRLBatchedEnv) -> torch.Tensor:
        if state["q_ref"] is None:
            state["q_ref"] = env.q.clone()
        else:
            # Refresh q_ref for envs that just reset (t reset to 0)
            need_refresh = env.t == 0
            if need_refresh.any():
                state["q_ref"][need_refresh] = env.q[need_refresh]
