"""Step-level state-conditional RL ↔ Classical switching.
...
Usage:
    python -m Yuan.RL_controller.eval.hybrid \\
        ...
        --out Yuan/RL_controller/runs/p0_progress_only_30M_0520/diag_10000_classical/hybrid.npz
"""
...
def main():
    ...
    n_pairs = len(pairs)
    print(f"[hybrid] N_tasks={n_tasks}  n_pairs={n_pairs}  "
          f"chunk={args.pairs_per_chunk}  device={device}")
    print(f"[hybrid] pairs (enter, exit):")
    for te, tx in pairs:
    ...
    for ck, chunk_pairs in enumerate(chunks):
        ...
        dt_run = time.time() - t0
        print(f"[hybrid]   chunk done in {dt_run:.1f}s")
    ...
    print()
    print(f"[hybrid] best hybrid: {best_b['name']}  "
    ...
    if args.out:
        ...
        print(f"[hybrid] saved raw → {args.out}")
    ...
    if args.out_csv:
        ...
        print(f"[hybrid] saved csv → {args.out_csv}")
