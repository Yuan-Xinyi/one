...
2. `env/ik_init.py` — 给定 line 起点位置 $p_0$，求一个可行 q₀
...
5. `train.py` — PPO 训练入口
6. `eval.py` — 评估脚本，输出 rollout 长度 ratio (RL / baseline)
...
$$
- $\hat u$：直线单位方向；$v$：恒定线速度（超参，默认 $v = 0.05$ m/s）
...
设计原则：position-only 任务下 EE 位置由 $J_p^+ \dot x_{task}$ 闭式决定，与 $a$ 无关 → 不存在回头路 → "走得远"等价于"活得久"。RL 唯一的优化目标是最大化 episode 长度，单项 alive reward 即可表达。

| 项 | 公式 |
|---|---|
| 存活 | $r_{alive} = w_{alive}$（per-step 常量，默认 1.0） |

终止时按 §终止条件 表里的惩罚（自碰撞 −100、锥越界 / JL −50）；success 终止不加 bonus。
...
|---|---|---|
| line 走完 $s_t \geq s_{total}$（success） | 0 | terminated=True |
| 自碰撞 | -100 | terminated=True |
| 旋转锥越界 $\angle(z_t, n_{target}) > 30°$ | -50 | terminated=True |
| 任一关节极限触发 | -50 | terminated=True |
| `step_count ≥ max_steps` | 0 | truncated=True |

- $n_{target}$ 是 line spec 提供的目标法向（episode 内恒定）。agent 需用 4-DOF nullspace 让 $z_t$ 不超出 30° 锥
- success 终止不加 bonus（agent 已通过 $r_{alive}$ 累积奖励；额外 bonus 会让 V function 在边界跳变）
- `max_steps` 默认设为覆盖最长 line：$\lceil 1.5 \cdot L_{max} / (v \cdot dt) \rceil = \lceil 1.5 \cdot 1.2 / 0.0005 \rceil = 3600$，向上取 4000

**Gymnasium 语义**：`terminated` / `truncated` 严格区分。**PPO 下 truncated 必须 bootstrap**（$V(s_T)$ 加入 advantage 计算）；success 终止也按 terminated 处理（不 bootstrap）。
...
给定 line 起点位置 $p_0$（姿态自由），调用 `SELIKSolver`（position-only 模式）解可行 $q_0$：
...
- 任一失败：换 seed 重试若干次；全部失败则报错（不静默 fallback）
...
obs = concat(
    q_normalized,               # 7,  (q - q_mid) / q_half_range ∈ [-1,1]^7
                                #     |q_i_norm| → 1 即接近 JL，编码了原 jl_distances
    progress_ratio,             # 1,  s/s_total ∈ [0,1]，success 时刚好达 1.0
    remaining_distance_vec,     # 3,  (line_end - p_ee) / L_total，世界系，单位无量纲
    z_tool,                     # 3,  当前工具轴单位向量
...
- 终止条件与 RL 环境完全一致（自碰撞 / 30° 锥 / JL / max_steps / success）
...
- mini-batch：`batch_size = 128`，`n_epochs = 10`
- 学习率：actor + critic 共用 3e-4（可加 linear schedule）
- `ent_coef = 0.0`；`vf_coef = 0.5`；`max_grad_norm = 0.5`
- 网络：三层 MLP `[256, 256, 256]`，ReLU；actor 独立 head 输出 $(\mu, \log\sigma)$
...
- 单独写 `LineDistribution` 类，方便替换分布

---

## 评估
- 固定 holdout 集：200 条 lines，seed 固定，可重现
- 不报 success_rate / 绝对 mean_len——只看 ratio
- 与 baseline 对比：手工 GPM nullspace controller（见 §Baseline）在同一 200 条 lines 上的 rollout 长度作分母
- `eval.py` 输出 CSV：`line_id, L_rl, L_baseline, ratio, term_reason_rl, term_reason_baseline, mean_sigma_min_rl`
...
- 不报 success_rate / 绝对长度均值，只报 ratio

---

## README 末尾 "Open Questions"（实现者给初步判断 + 不确定度，不是问用户）
1. $w_{alive}$ 量级如何选？默认 1.0 配合终止惩罚 −50 / −100 是否合理？典型 success episode 累积 reward 与终止惩罚的比例如何？
2. MC reachability 点云规模 $10^5$ 够用？line 起点采样的覆盖均匀度如何？
3. 预期 rollout 长度 ratio (RL / baseline) 能达到多少？
