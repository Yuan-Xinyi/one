...
## 补充 4(0707):框架简化定稿——回归最简 expert iteration(用户决策)

**动机(用户)**:种子选择被证明是主要提升来源(论文口径 +7.2pp vs 控制器侧 +0.8pp,其中前瞻搜索仅 +0.4pp),为 0.4pp 背一整套搜索机制不划算。**决策:删除 MPC/前瞻搜索,专家=混合策略本身**(边界区域 classical、内部区域当前学生、窄带线性混合),学生=与 RL actor 同结构网络,DAgger+热启动+末两代参数平均保留。

**定稿数字(论文口径,冻结参考值)**:

| 行 | 含搜索 | 简化版(定稿) | 损失 |
|---|---|---|---|
| π_D 单独运行 | 89.4 | **89.0**(79.8/91.0/95.0) | −0.4 |
| 混合系统 | 91.4 | **91.0**(82.9/92.6/96.4) | −0.4 |
| 完整方法 | 98.38 | **98.19**(93.7/98.8/101.6) | −0.19 |
| best-of-25 上限 | 103.22 | 102.89 | — |

内部指标:简化版学生 = avg(r5,r6) = `distill_simple_exit_final`,pure 1.045 / hyb 1.058(0.985/0.96)。
**打分器**:候选与特征与控制器无关——选择结果完全一致,旧打分器直接迁移得 97.91;用简化控制器重新生成 6 万任务标签重训后 **98.19**(兑现比例 60.5% 不变),`rank_train/ranker_simple.pt`。
**全部配套重测**(简化控制器下,均与原结论一致):失败归因(411 任务、cone 66.2%、换种子 71% 恢复≥90%)、候选可交换性(90.99≈90.93)、best-of-K 前缀(95.2/98.3/100.3)、消融行(排序损失 98.2 vs 逐点回归 96.0;候选批 97.4/98.0/98.2)、holdout(90.3/98.58/102.83,配对 +8.32±0.56)、噪声鲁棒(σ0.02:61.9 vs 57.2)。缓存:`rank_phase0/simple_ctrl/`、`fresh_holdout_simple/`、`distill_simple_exit_final/tail_autopsy.npz`。
**论文**:摘要/引言/相关工作/IV-D("Internalizing the Hybrid via Expert Iteration")/Algorithm 2(缩为 4 行行为地图)/表 III/IV/V/VI/结论全部换为简化框架与新数字;前瞻搜索以三句话保留为 exploratory experiment(+0.4pp,机制复杂度不成比例);MuZero/Dreamer 讨论段更新为"最终方法完全无搜索"。含搜索的全部工件(r7-r13)保留在 runs/ 供查。
