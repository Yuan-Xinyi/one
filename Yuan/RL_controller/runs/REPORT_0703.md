...
## 补充 5(0707):第 7-12 轮迭代——用户假设证实,"多迭代替代搜索"

**动机(用户)**:切换仍有帮助的比例在第 6 轮后仍在下降,应继续迭代验证是否真收敛。
**轨迹(内部指标,每轮采集 1.2 万任务、DAgger 聚合,数据集 216 万→484 万行)**:

| 轮 | 6 | 7 | 8 | 9 | 10 | 11 | 12 | 定稿 avg(11,12) |
|---|---|---|---|---|---|---|---|---|
| 单独运行 | 1.0446 | 1.0455 | 1.0457 | 1.0490 | 1.0491 | 1.0496 | 1.0497 | **1.0498** |
| 切换有帮助比例 | 8.5% | 7.3% | 7.3% | 6.8% | 6.5% | 6.2% | 6.1% | 6.2% |

- 第 6 轮"已收敛"的判断被推翻:vs 第 6 轮 +0.0043±0.0015(显著);真收敛点在第 11-12 轮(增量 0.0001);
- **无搜索学生 1.0498 超过含搜索对应产物 soup2(1.0492)**——搜索教师的提升可被更多轮次基本替代(收敛后剩余差距:单独运行 0.3pp、系统 0.4pp,论文口径);
- 定稿学生 = `exit_rounds7plus/final_avg`(r11+r12 参数平均;r9r10 平均备份在 final_avg_r9r10)。

**定稿级联重测(全部通过)**:论文口径单独运行 **89.1**(80.1/91.1/94.8)、系统 **91.0**(82.7/92.7/96.3)、first-valid 90.87、**完整方法 98.17**(93.9/98.7/101.6,配对 +7.30±0.23)、上限 102.88;打分器重训(`ranker_final.pt`,val capture 64.7%);holdout 90.15/98.30/102.77(+8.15±0.54),噪声 σ0.02:63.5 vs 57.4;归因(428 任务,cone 68.2/jl 19.9/coll 11.2,42.8%+66.3% 换种子恢复);消融(逐点 96.0 vs 排序 98.2;候选批 97.4/97.9/98.2)。缓存:`rank_phase0/final_ctrl/`、`fresh_holdout_final/`、`exit_rounds7plus/final_avg/tail_autopsy.npz`。
**论文第三次更新**:迭代收敛描述改为"约十二轮、末两轮变化 <0.1pp";探索性搜索备注改写为"收敛后仅领先 0.3/0.4pp——多迭代可基本替代搜索";全部表格与正文数字换为第 12 轮定稿值。
