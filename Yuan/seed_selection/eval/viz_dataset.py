...
Usage:
    # NPZ mode
    python -m Yuan.seed_selection.eval.viz_dataset \\
        --npz Yuan/seed_selection/runs/pilot_20k/pilot_20k.npz --task 11

    # RAW mode (label pipeline runs live; ~10-60s/task on cuda)
    python -m Yuan.seed_selection.eval.viz_dataset --seed 42
"""
...
_DEFAULT_NPZ = _REPO_ROOT / "Yuan/seed_selection/runs/pilot_20k/pilot_20k.npz"
_DEFAULT_CONFIG = _REPO_ROOT / "Yuan/RL_controller/config.yaml"

parser = argparse.ArgumentParser()
parser.add_argument("--npz", default=str(_DEFAULT_NPZ),
                    help="dataset NPZ (from dataset_builder). Pass 'RAW' for live label pipeline.")
...
def _sample_raw(seed: int, config_path: str, n_pool: int, n_envs: int):
    ...
    from Yuan.seed_selection.smm.label_builder import build_labels_for_one_task
...
_npz_arg = None if (args.npz is None or str(args.npz).upper() == "RAW") else args.npz
task = _load_from_npz(_npz_arg, args.task) if _npz_arg else \
