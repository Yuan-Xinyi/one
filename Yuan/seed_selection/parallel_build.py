...
`Yuan.seed_selection.speedtest` on its slice with an isolated cache_name),
waits for all to finish, then concatenates the K chunk NPZs into a single
final NPZ at `<out_dir>/<cache_name>.npz`.
...
Usage:
    python -m Yuan.seed_selection.parallel_build \\
...
def parse_args():
    ...
    p.add_argument('--total-n-tasks', type=int, required=True)
...
def merge_chunk_npzs(chunk_paths: list[Path], out_path: Path) -> None:
    ...
    from Yuan.seed_selection.dataset_builder import _compute_norm_stats
...
def main():
    ...
    for c in to_launch:
        ...
        cmd = [
            sys.executable, '-u', '-m', 'Yuan.seed_selection.speedtest',
            '--n-tasks', str(c['n']),
            '--seed', str(c['seed']),
            '--cache-name', c['cache'],
