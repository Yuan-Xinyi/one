...
ext_npz = P0DIR / 'candidates_ext8.npz'
if ext_npz.exists():
    d = np.load(ext_npz)
    ...
    seeds, ik_ok = diffusion_seeds(
        ...
        cfg_w=1.5, sample_seed=9600, kin=env.kin, device=dev,
...
for si in range(K_EXT):
    slot_npz = P0DIR / f'L_slot{8 + si}.npz'
    if slot_npz.exists():
        continue
    ...
    print(f'[k16] slot {8+si} done', flush=True)

feat_npz = P0DIR / 'feat_v2_ext8.npz'
