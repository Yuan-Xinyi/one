...
START = RUNS / 'exit_rounds7plus/round10'      # continue from round 10
...
for _r in range(7, 11):
    _d = np.load(RUNS / f'exit_rounds7plus/round{_r}/dataset.npz')
    D_obs.append(torch.from_numpy(_d['obs']).float())
    D_act.append(torch.from_numpy(_d['act']).float())
