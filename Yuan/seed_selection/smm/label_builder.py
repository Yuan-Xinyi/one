...
from Yuan.seed_selection.smm.robustness import filter_robust_candidates
from Yuan.seed_selection.smm.rollout import DEFAULT_TARGET_DISTANCE_M
...
def _candidates_from_rollout(Q_candidates_t: torch.Tensor,
                              score_res: dict,
                              branch_full: np.ndarray) -> list[dict]:
    """Pack a batched-rollout result into per-candidate dicts."""
    ...
    for j in range(Q_candidates_t.shape[0]):
        out.append({
            ...
            "max_q_norm": (float(max_q_norm_arr[j])
                           if max_q_norm_arr is not None else float("nan")),
            "branch_id": int(branch_full[j]),
        })
    return out


def _collect_branch_stats(extras: dict, candidates: list[dict],
                           n_branches: int) -> None:
    """Populate the per-branch L distribution + q0_seed_branch_rank in ``extras``.

    Only candidate slots with a non-negative ``branch_id`` count (q0_seed at
    index 0 has branch_id = -1 and is skipped). Branches beyond
    ``MAX_BRANCHES_PERSIST`` are summarized but not persisted.
    """
    if n_branches <= 0:
        return
    branch_Ls: list[list[float]] = [[] for _ in range(n_branches)]
    for cd in candidates[1:]:
        bi = cd["branch_id"]
        if 0 <= bi < n_branches:
            branch_Ls[bi].append(cd["L_clean"])
    branch_max_L = np.full(n_branches, -np.inf, dtype=np.float32)
    for bi in range(min(n_branches, MAX_BRANCHES_PERSIST)):
        Ls = np.asarray(branch_Ls[bi], dtype=np.float32)
        extras["branch_sizes"][bi] = int(Ls.size)
        if Ls.size == 0:
            continue
        ...
        branch_max_L[bi] = float(Ls.max())
    seed_bi = extras["q0_seed_branch_id"]
    if seed_bi >= 0 and np.any(np.isfinite(branch_max_L)):
        order = np.argsort(-branch_max_L)
        rank_of = {int(bi): r for r, bi in enumerate(order)}
        extras["q0_seed_branch_rank_by_L"] = int(rank_of.get(seed_bi, -1))


def _record_top_kprime(extras: dict, above_floor: list[dict],
                        K_prime: int) -> list[dict]:
    """Sort ``above_floor`` by L_clean desc, persist the top K' to extras,
    and return the (truncated) sorted list."""
    top = sorted(above_floor, key=lambda x: -x["L_clean"])[:K_prime]
    for ki, cd in enumerate(top):
...
def _apply_seed_label_extras(extras: dict, seed_cand: dict) -> None:
    """Write the q0_seed (fallback) candidate's term_reason / n_steps /
    max_q_norm into slot 0 of the per-label extras arrays."""
    extras["labels_term_reason"][0] = _term_str(seed_cand["term_reason"])
    extras["labels_n_steps"][0] = int(seed_cand["n_steps"])
    extras["labels_max_q_norm"][0] = float(seed_cand["max_q_norm"])


def _build_fallback_result(*, status: str, L_seed: float, q0_seed_t: torch.Tensor,
                            seed_cand: dict, extras: dict, diag: dict,
                            t_pipeline_start: float,
                            Q_candidates_t: torch.Tensor | None = None,
                            score_res: dict | None = None,
                            return_all_candidates: bool = False) -> dict:
    """Build the result dict for the q0_seed-only fallback path.

    Used by both the infeasible-task and the robust-filter-empty branches.
    ``Q_candidates_t``/``score_res`` are only consulted when
    ``return_all_candidates`` is True.
    """
    _apply_seed_label_extras(extras, seed_cand)
    ...
    if return_all_candidates and Q_candidates_t is not None and score_res is not None:
        result["all_candidates_q0"] = Q_candidates_t.detach().cpu().clone()
        result["all_candidates_L"] = [float(x) for x in score_res["L"].tolist()]
    return result


def _apply_label_extras(extras: dict, labels: list[dict]) -> None:
    """Copy each label's term_reason / n_steps / max_q_norm into the per-label
    extras arrays. Robust-filter outputs may omit these — defaults stay if so.
    """
    for li, lab in enumerate(labels):
        tr = lab.get("term_reason")
        ns = lab.get("n_steps")
        mq = lab.get("max_q_norm")
        if tr is not None:
...
def _classify_status(*, fallback_used: bool, max_L: float, n_branches: int,
                      L_min_acceptable: float, edge_branch_threshold: int) -> str:
    """Map (fallback / max_L / branch count) to the four-way status taxonomy."""
    if fallback_used:
        return STATUS_EDGE if max_L >= L_min_acceptable else STATUS_LOW_QUALITY
    if n_branches <= edge_branch_threshold:
        return STATUS_EDGE
...
def build_labels_for_one_task(
    ...
    *,
    ...
    candidates = _candidates_from_rollout(Q_candidates_t, score_res, candidate_branch_full)
    ...
    _collect_branch_stats(extras, candidates, n_branches)
    ...
    if len(above_floor) == 0:
        ...
        return _build_fallback_result(
            status=STATUS_INFEASIBLE, L_seed=L_seed, q0_seed_t=q0_seed_t,
            seed_cand=candidates[0], extras=extras, diag=diag,
            t_pipeline_start=t_pipeline_start,
            Q_candidates_t=Q_candidates_t, score_res=score_res,
    ...
    top_for_kprime = _record_top_kprime(extras, above_floor, K_prime)
    ...
    if len(passed) == 0:
        ...
        fallback = _build_fallback_result(
            status=STATUS_KEPT, L_seed=L_seed, q0_seed_t=q0_seed_t,  # status fixed below
            seed_cand=candidates[0], extras=extras, diag=diag,
            t_pipeline_start=t_pipeline_start,
            Q_candidates_t=Q_candidates_t, score_res=score_res,
            return_all_candidates=return_all_candidates,
        )
        fallback["status"] = _classify_status(
            fallback_used=True, max_L=float(L_seed), n_branches=n_branches,
            L_min_acceptable=L_min_acceptable,
            edge_branch_threshold=edge_branch_threshold,
        )
        return fallback
    ...
    _apply_label_extras(extras, labels)

    # ---------- 11. Status determination ----------
    status = _classify_status(
        fallback_used=False, max_L=max(labels_L_clean) if labels_L_clean else 0.0,
        n_branches=n_branches, L_min_acceptable=L_min_acceptable,
        edge_branch_threshold=edge_branch_threshold,
    )
