"""Speed test for dataset build: time N-task build with seed 0.

Mirrors `tests/pilot_day5.py` exactly except:
  * N is configurable via --n-tasks (default 100)
  * --seed selects which slice of the cached line_distribution to use
  * cache_name is isolated ("speedtest_n{N}_s{seed}") so it never collides
    with pilot_1k partials.

Usage:
    time python -m Yuan.seed_selection.speedtest --n-tasks 100 --seed 0
"""
...
# Match tests/pilot_day5.py exactly.
LABEL_KWARGS = dict(
    cone_angle_deg=5.0,
    n_orientations=10,
    n_ik_restarts=5,
